		...
		if (currentItem.image) {
			return (
				<div className="relative h-full w-full overflow-hidden">
					{/* Previous image as backdrop for smooth transitions */}
					{previousImage && (
						<img
							src={previousImage}
							alt=""
							className="absolute inset-0 h-full w-full rounded-xl border border-neutral-300/50 object-cover"
						/>
					)}
					...
					<motion.img
						...
						className={cn(
							"absolute inset-0 h-full w-full rounded-xl border border-neutral-300/50 object-cover",
						...
						onLoad={() => {
							setImageLoaded(true)
							setPreviousImage(currentItem.image ?? null)
						}}
						loading="eager"
					/>
				</div>
		...
		<div ref={ref} className="w-full">
			<div className="flex w-full flex-col items-center justify-center max-w-7xl mx-auto">
				<div className="grid h-full grid-cols-5 gap-x-10 px-10 md:px-20 items-center w-full">
					...
					<div
						className={`col-span-5 h-[350px] lg:h-[400px] w-auto lg:col-span-3 ${
							ltr && "md:order-1"
						}`}
					>
						{renderMedia()}
